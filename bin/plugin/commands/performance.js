...
async function runPerformanceTests( revs, options ) {
	...
	await runShellScript( 'npm ci && npm run build:packages', testsPath );
	log( '    >> Creating the environment folders' );
	await runShellScript( `mkdir -p ${ rootPath }/envs` );

	// 2- Preparing the environment directories per revision.
	...
	for ( const rev of revs ) {
		...
		log( `        >> Checking out revision ${ formats.success( rev ) }` );
		// @ts-ignore
		await SimpleGit( buildPath )
			.reset( 'hard' )
			.checkout( `branch-${ rev }` );

		log( `        >> Building revision ${ formats.success( rev ) }` );
